...
NSString * const YYModelTransformationErrorDomain = @"YYModelTransformationErrorDomain";
const NSInteger YYModelTransformationErrorNoClassFound = 2;
const NSInteger YYModelTransformationErrorInvalidJSONDictionary = 3;
// An exception was thrown and caught.
const NSInteger YYModelTransformationErrorExceptionThrown = 1;
...
@interface _YYModelPropertyMeta : NSObject {
    ...
    BOOL _isAutoUnderscoreCaseKey;  /// 是否是自动下划线转驼峰Key
}
@end
...
+ (instancetype)metaWithClassInfo:(YYClassInfo *)classInfo propertyInfo:(YYClassPropertyInfo *)propertyInfo generic:(Class)generic {
    ...
    _YYModelPropertyMeta *meta = [[self alloc] init];
                        ...
                        if (valueArr) {
                            ...
                            for (id one in valueArr) {
                                if ([one isKindOfClass:meta->_genericCls]) {
                                    ...
                                    [newOne yy_modelSetWithDictionary:one error:error];
                        ...
                        if (meta->_genericCls) {
                            ...
                            [((NSDictionary *)value) enumerateKeysAndObjectsUsingBlock:^(NSString *oneKey, id oneValue, BOOL *stop) {
                                if ([oneValue isKindOfClass:[NSDictionary class]]) {
                                    ...
                                    NSObject *newOne = [cls new];
                                    [newOne yy_modelSetWithDictionary:(id)oneValue error:error];
        ...
        switch (meta->_type & YYEncodingTypeMask) {
            case YYEncodingTypeObject: {
                if (isNull) {
                    ...
                    if (one) {
                        [one yy_modelSetWithDictionary:value error:error];
                    } else {
                        Class cls = meta->_cls;
                        if (meta->_hasCustomClassFromDictionary) {
...
typedef struct {
    ...
    NSError *__autoreleasing * _Nullable error;     ///< NSError
} ModelSetContext;
...
static NSString * camelCaseKeyWithUnderscoreCaseKey(NSString * underscoreCaseKey) {
    //bail early if no transformation required
    if ([underscoreCaseKey rangeOfString:@"_"].location==NSNotFound) return underscoreCaseKey;
    
    //derive camel case out of underscore case
    NSString* camelCaseKey = [underscoreCaseKey capitalizedString];
    camelCaseKey = [camelCaseKey stringByReplacingOccurrencesOfString:@"_" withString:@""];
    camelCaseKey = [camelCaseKey stringByReplacingCharactersInRange:NSMakeRange(0, 1) withString:[[camelCaseKey substringToIndex:1] lowercaseString] ];
    
    return camelCaseKey;
}


static void YYModelSetValueForProperty(_YYModelPropertyMeta *propertyMeta, const void *_value, id model, NSError *__autoreleasing * _Nullable error) {
    while (propertyMeta) {
        if (propertyMeta->_setter) {
            ModelSetValueForProperty(model, (__bridge __unsafe_unretained id)_value, propertyMeta, error);
        }
...
static void ModelSetWithDictionaryFunction(const void *_key, const void *_value, void *_context) {
    ...
    NSError *__autoreleasing * _Nullable error = context->error;
    
    if (propertyMeta) {
        YYModelSetValueForProperty(propertyMeta, _value, model, error);
    } else {
        if (model &&
            [[model class] respondsToSelector:@selector(mapperType)] &&
            [[model class] mapperType] == YYMapperTypeUnderscoreCaseToCamelCase &&
            [(__bridge id)_key isKindOfClass:NSString.class]) {
            
            propertyMeta = [meta->_mapper objectForKey:camelCaseKeyWithUnderscoreCaseKey(((NSString *)(__bridge id)_key))];
            if (propertyMeta) {
                propertyMeta->_isAutoUnderscoreCaseKey = YES;
                YYModelSetValueForProperty(propertyMeta, _value, model, error);
            } else {
                ...
                if (error != NULL) {
                    if ((*error).userInfo) {
                        ...
                        [userInfo setObject:[NSString stringWithFormat:@"Caught exception parsing JSON key path \"%@\" for model class: %@", ((NSString *)(__bridge id)_key), [model class]] forKey:((NSString *)(__bridge id)_key)];
                        [(*error) setValue:userInfo forKeyPath:@"userInfo"];
                    } else {
                        userInfo = [NSMutableDictionary dictionary];
                        [userInfo setObject:[NSString stringWithFormat:@"Caught exception parsing JSON key path \"%@\" for model class: %@", ((NSString *)(__bridge id)_key), [model class]] forKey:((NSString *)(__bridge id)_key)];
                        *error = [NSError errorWithDomain:YYModelTransformationErrorDomain code:YYModelTransformationErrorExceptionThrown userInfo:userInfo];
                    }
                }
            }
        }
...
static void ModelSetWithPropertyMetaArrayFunction(const void *_propertyMeta, void *_context) {
    ...
    NSError *__autoreleasing * _Nullable error = context->error;
    if (value) {
        ModelSetValueForProperty(model, value, propertyMeta, error);
    } else {
        if (model &&
            [[model class] respondsToSelector:@selector(mapperType)] &&
            [[model class] mapperType] == YYMapperTypeUnderscoreCaseToCamelCase &&
            [propertyMeta->_mappedToKey isKindOfClass:NSString.class] &&
            [propertyMeta->_mappedToKey rangeOfString:@"_"].location == NSNotFound) { // 不包含_
            value = [dictionary objectForKey:underscoreCaseKeyWithCamelCaseKey(propertyMeta->_mappedToKey)];
            if (value) {
                propertyMeta->_isAutoUnderscoreCaseKey = YES;
                ModelSetValueForProperty(model, value, propertyMeta, error);
            } else {
                ...
                if (error != NULL) {
                    if ((*error).userInfo) {
                        userInfo = ((*error).userInfo).mutableCopy;
                        [userInfo setObject:[NSString stringWithFormat:@"Caught exception parsing JSON key path \"%@\" for model class: %@", propertyMeta->_mappedToKey, [model class]] forKey:propertyMeta->_mappedToKey];
                        [(*error) setValue:userInfo forKeyPath:@"userInfo"];
                    } else {
                        userInfo = [NSMutableDictionary dictionary];
                        [userInfo setObject:[NSString stringWithFormat:@"Caught exception parsing JSON key path \"%@\" for model class: %@", propertyMeta->_mappedToKey, [model class]] forKey:propertyMeta->_mappedToKey];
...
static id ModelToJSONObjectRecursive(NSObject *model) {
    ...
    [modelMeta->_mapper enumerateKeysAndObjectsUsingBlock:^(NSString *propertyMappedKey, _YYModelPropertyMeta *propertyMeta, BOOL *stop) {
        ...
        if (propertyMeta->_mappedToKeyPath) {
            ...
            if (propertyMeta->_mappedToKey && !dic[propertyMeta->_mappedToKey]) {
                if (propertyMeta->_isAutoUnderscoreCaseKey) {
                    dic[underscoreCaseKeyWithCamelCaseKey(propertyMeta->_mappedToKey)] = value;
                } else {
...
+ (instancetype)yy_modelWithJSON:(id)json error:(NSError **)error {
    NSDictionary *dic = [self _yy_dictionaryWithJSON:json];
    return [self yy_modelWithDictionary:dic error:error];
}

+ (instancetype)yy_modelWithDictionary:(NSDictionary *)dictionary {
    return [self yy_modelWithDictionary:dictionary error:nil];
}

+ (instancetype)yy_modelWithDictionary:(NSDictionary *)dictionary error:(NSError **)error {
    ...
    if ([one yy_modelSetWithDictionary:dictionary error:error]) return one;
    return nil;
}
...
- (BOOL)yy_modelSetWithJSON:(id)json error:(NSError **)error {
    NSDictionary *dic = [NSObject _yy_dictionaryWithJSON:json];
    return [self yy_modelSetWithDictionary:dic error:error];
}

- (BOOL)yy_modelSetWithDictionary:(NSDictionary *)dic {
    return [self yy_modelSetWithDictionary:dic error:nil];
}

- (BOOL)yy_modelSetWithDictionary:(NSDictionary *)dic error:(NSError **)error {
...
+ (YYMapperType)mapperType {
    return YYMapperTypeNone;
}
...
+ (NSArray *)yy_modelArrayWithClass:(Class)cls json:(id)json {
    return [self yy_modelArrayWithClass:cls json:json error:nil];
}

+ (NSArray *)yy_modelArrayWithClass:(Class)cls json:(id)json error:(NSError **)error {
    if (!json) return nil;
    NSArray *arr = nil;
    NSData *jsonData = nil;
    ...
    return [self yy_modelArrayWithClass:cls array:arr error:error];
...
+ (NSArray *)yy_modelArrayWithClass:(Class)cls array:(NSArray *)arr error:(NSError **)error {
    if (arr == nil || ![arr isKindOfClass:NSArray.class]) {
        if (error != NULL) {
            NSDictionary *userInfo = @{
                                       NSLocalizedDescriptionKey: NSLocalizedString(@"Missing JSON array", @""),
                                       NSLocalizedFailureReasonErrorKey: [NSString stringWithFormat:NSLocalizedString(@"%@ could not be created because an invalid JSON array was provided: %@", @""), NSStringFromClass(cls), arr.class],
                                       };
            *error = [NSError errorWithDomain:YYModelTransformationErrorDomain code:YYModelTransformationErrorInvalidJSONDictionary userInfo:userInfo];
        }
        return nil;
    }
    ...
    if (!cls ) {
        if (error != NULL) {
            ...
            *error = [NSError errorWithDomain:YYModelTransformationErrorDomain code:YYModelTransformationErrorNoClassFound userInfo:userInfo];
        }
        return nil;
    }
    NSMutableArray *result = [NSMutableArray new];
    for (NSDictionary *dic in arr) {
        if (![dic isKindOfClass:[NSDictionary class]]) continue;
        NSObject *obj = [cls yy_modelWithDictionary:dic error:error];
        if (obj) [result addObject:obj];
    }
    return result;
}
